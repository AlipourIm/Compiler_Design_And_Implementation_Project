...
function runAnimation(queue) {
	...
	setTimeout(function() {
		...
		logResult(getStateByID(queue[queue.length - 1].stateID).name, getStateByID(queue[queue.length - 1].stateID).label, 
...
function isAutomataComplete() {
	...
	for (var i = 0; i < stateList.length; i++) {
		...
		if (exitSymbols.length != 0 && stateList[i].end == false) {
...
function logResult(final_state_name, final_state_label, accept) {
	if (firstResult) {
		firstResult = false;
		$("#results").html("<br><b>RESULTS:</b><br>");
	}

	input = final_state_name.substr(-1) == "*" ? runInfo.input.slice(0,-1) : runInfo.input; 
	var txt = "[input: \"<b>" + input + "</b>\", ";
	if (accept) {
		txt += "end state: <b>" + final_state_name + "</b>] - accepted. (" + final_state_label + ") <br><br>";
	}
	else {
